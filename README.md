...
| 准备场景资产 | [场景包说明](docs/scene/scene-package.md) -> [视频到 USD 场景](docs/scene/video-to-usd.md) |
...
```text
.
├── README.md                       本仓库 / GitBook 首页
├── SUMMARY.md                      GitBook 目录
├── docs/                           文档源文件
│   ├── overview/                   概览
│   ├── setup/                      环境与导入
│   ├── scene/                      场景与资产
│   ├── ros2-integration/           ROS2 集成
│   ├── debugging/                  调试与可视化
...
> 这个仓库**只装文档**,不包含项目源码。源码仓公开后再补充 clone 地址。

---

## 在 GitBook 上托管

主线方案是把这个仓库连接到 GitBook,它会自动:
...
2. 在 Space 设置里选 **Sync with Git -> GitHub**,授权 GitBook 读取 _待补充的文档仓库_。
...
`mkdocs.yml` 里的导航跟 [`SUMMARY.md`](SUMMARY.md) 保持同步。
